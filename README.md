# Shapez.io Multiplayer Mod (v1.5.0)

A real-time cooperative multiplayer mod for shapez.io.
...
Share a single factory map with friends, build cooperative infrastructure, and deliver shapes to a shared hub. The project consists of a **client-side mod** injected into the game and an **Electron-based Launcher** that serves as both the installer and the embedded relay server.
...
| `launcher/main.js` | Main process: mod installer, WebSocket relay server, auto-updater |
| `launcher/index.html` | Launcher UI |
| `launcher/renderer.js` | Launcher renderer process logic |
| `launcher/preload.js` | Electron preload bridge |
| `shapez-multiplayer.js` | Client-side mod injected into shapez.io |
| `server.js` | **Deprecated** — standalone relay server kept for reference only |
...
The multiplayer ecosystem is entirely managed through the **Shapez Multiplayer Launcher**. This Electron application automatically installs the mod into your local game directory and runs an embedded WebSocket relay server — no separate server setup is required.
...
5. If you intend to host the session, click **Start Server** directly within the Launcher. The relay server starts on port `3005` by default.
